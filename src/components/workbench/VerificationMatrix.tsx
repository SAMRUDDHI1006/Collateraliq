  ...
  return (
    <div className="flex flex-col h-full bg-slate-900 border border-slate-800 rounded-lg overflow-hidden text-slate-100 text-xs select-none shadow-md min-h-0">
      ...
      <div className="shrink-0 h-8 px-3 bg-slate-800/80 border-b border-slate-700/60 flex items-center justify-between text-[11px]">
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          <span className={`font-mono text-[10px] font-bold px-1.5 py-0.2 rounded ${
            Math.abs(pctDiff) > 8 ? 'bg-rose-950 text-rose-300 border border-rose-800' :
            Math.abs(pctDiff) > 3 ? 'bg-amber-950 text-amber-300 border border-amber-800' :
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        ...
        {isBT && (
          <div className="bg-gradient-to-br from-amber-950/40 to-slate-950 rounded-xl border-2 border-amber-700/60 p-4 space-y-3">
            <div className="flex items-center justify-between text-xs font-bold">
              <div className="flex items-center gap-1.5 text-amber-400">
                <TrendingUp className="w-4 h-4" />
                <span className="uppercase tracking-wider text-[11px]">Balance Transfer — Valuation History</span>
              </div>
              <span className="font-mono text-[10px] px-2 py-0.5 rounded bg-amber-950 text-amber-300 border border-amber-700">
                3-Value Comparison
              </span>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 text-[10px] font-mono text-center">
              <div className="p-3 rounded-xl bg-slate-900 border border-slate-700 space-y-1">
                <span className="text-slate-400 block text-[9px] uppercase tracking-wider">Previous Bank</span>
                <span className="font-extrabold text-white text-sm block">{prevVal > 0 ? fmtCr(prevVal) : '—'}</span>
                <span className="text-slate-500 text-[9px]">{balanceTransfer?.previousLender || 'Previous Lender'}</span>
                {balanceTransfer?.previousValuationDate && (
                  <span className="text-slate-500 text-[9px] block">{balanceTransfer.previousValuationDate}</span>
                )}
              </div>
              <div className="p-3 rounded-xl bg-emerald-950/40 border border-emerald-700/60 space-y-1">
                <span className="text-slate-400 block text-[9px] uppercase tracking-wider">CollateralIQ Current</span>
                <span className="font-extrabold text-emerald-400 text-sm block">{fmtCr(modelIndicativeValue)}</span>
                <span className="text-slate-500 text-[9px]">AI Model Estimate</span>
              </div>
              <div className="p-3 rounded-xl bg-blue-950/40 border border-blue-700/60 space-y-1">
                <span className="text-slate-400 block text-[9px] uppercase tracking-wider">Current Valuer</span>
                <span className="font-extrabold text-blue-300 text-sm block">{valuerValue > 0 ? fmtCr(valuerValue) : '—'}</span>
                <span className="text-slate-500 text-[9px] truncate">{valuerReport?.valuerName ? valuerReport.valuerName.split('(')[0].trim() : 'Independent Valuer'}</span>
              </div>
            </div>

            {/* Flow Arrow */}
            <div className="flex items-center justify-center gap-1 text-slate-500 text-[10px] font-mono">
              <span className="text-slate-400">Previous Bank</span>
              <ArrowRight className="w-3 h-3" />
              <span className="text-emerald-400">CollateralIQ</span>
              <ArrowRight className="w-3 h-3" />
              <span className="text-blue-300">Valuer</span>
            </div>

            {/* Movement Calculations */}
            <div className="text-[11px] font-mono space-y-2 bg-slate-900 p-3 rounded-xl border border-slate-800">
              {prevVal > 0 ? (
                <>
                  <div className="flex items-center justify-between">
                    <span className="text-slate-400">Previous → Current Valuer:</span>
                    <span className={`font-bold ${prevToValuerAbs >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                      {fmtL(prevToValuerAbs)} ({fmtPct(prevToValuerPct)})
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-slate-400">Previous → CollateralIQ Current:</span>
                  ...
                  <div className="flex items-center justify-between border-t border-slate-700 pt-2">
                    <span className="text-slate-400">CollateralIQ ↔ Valuer Deviation:</span>
            ...
            {(balanceTransfer?.previousRatePerSqFt || balanceTransfer?.previousValuationMethod) && (
              <div className="grid grid-cols-2 gap-2 text-[10px] font-mono bg-slate-900 p-2 rounded-lg border border-slate-800">
                {balanceTransfer?.previousRatePerSqFt && (
                  <span className="text-slate-400">Prev Rate: <span className="text-white font-bold">₹{balanceTransfer.previousRatePerSqFt.toLocaleString()}/sq.ft</span></span>
                )}
                {balanceTransfer?.previousAreaConsidered && (
                  <span className="text-slate-400">Prev Area: <span className="text-white font-bold">{balanceTransfer.previousAreaConsidered} sq.ft</span></span>
                )}
                {balanceTransfer?.previousValuationMethod && (
                  <span className="text-slate-400">Prev Method: <span className="text-white font-bold">{balanceTransfer.previousValuationMethod}</span></span>
                )}
                {balanceTransfer?.previousValuerName && (
                  <span className="text-slate-400 col-span-2">Prev Valuer: <span className="text-white font-bold">{balanceTransfer.previousValuerName}</span></span>
                )}
              </div>
            )}
          </div>
        ...
        <div className="bg-slate-950 rounded-xl border border-slate-800 p-3.5 space-y-2">
          ...
          {/* Review Level inline */}
          <div className={`inline-flex items-center gap-1.5 px-2 py-1 rounded font-mono text-[10px] font-bold border ${
        ...
        <div className="bg-slate-950 rounded-xl border border-slate-800 p-3.5 space-y-3">
          ...
          <div className="space-y-2 text-[11px]">
            ...
            <div className="p-2 rounded-lg bg-slate-900 border border-slate-800 flex justify-between items-center">
              ...
              <span className={`font-mono ${
                ...
                {deviation?.areaUsed?.modelArea === deviation?.areaUsed?.valuerArea ? ' (No material difference)' : ' ⚠ Mismatch'}
              </span>
            </div>
          ...
          {deviation?.explanationText && (
            <div className="p-2.5 rounded-lg bg-amber-950/20 border border-amber-800/40 text-[11px] text-amber-200 leading-relaxed">
              {deviation.explanationText}
            </div>
          )}
          ...
          {deviation?.identifiedDrivers && deviation.identifiedDrivers.length > 0 && (
            <div className="space-y-1">
              <span className="text-[10px] text-slate-400 uppercase tracking-wider font-mono">Identified Drivers:</span>
        ...
        {loanCase.reviewDrivers && loanCase.reviewDrivers.length > 0 && (
          <div className="bg-slate-950 rounded-xl border border-slate-800 p-3 space-y-2">
            <span className="uppercase tracking-wider text-[10px] text-slate-400 font-mono font-bold">Review Level Drivers</span>
            <div className="space-y-1">
