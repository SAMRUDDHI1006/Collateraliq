    ...
    <footer className="shrink-0 h-11 bg-slate-950 border-t border-slate-800 px-4 flex items-center justify-between z-30 select-none min-w-0">
      ...
      <div className="flex items-center gap-2 text-xs truncate">
        {!hasPendingExceptions ? (
          <div className="flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-emerald-950/80 border border-emerald-800 text-emerald-300 text-[11px] font-medium truncate">
            ...
            <span className="truncate">All Verification Checks Passed &bull; No Mandatory Exceptions Pending</span>
          </div>
        ) : exceptionAcknowledged ? (
          <div className="flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-emerald-950/80 border border-emerald-800 text-emerald-300 text-[11px] font-medium truncate">
            <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
            <span className="truncate">Area Discrepancy Exception Acknowledged &bull; Ready for Credit Sign-Off</span>
          </div>
        ) : (
          <div className="flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-amber-950/80 border border-amber-800 text-amber-300 text-[11px] font-semibold truncate animate-pulse">
            ...
            <span className="truncate">1 Mandatory Exception Pending Underwriter Review</span>
          </div>
        )}
      </div>
      ...
      <div className="flex items-center gap-2 shrink-0">
        <button
          ...
          className="px-3 py-1 rounded bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-semibold flex items-center gap-1.5 transition-colors border border-slate-700 shrink-0"
        >
        ...
        <button
          ...
          className="px-3 py-1 rounded bg-rose-950 hover:bg-rose-900 text-rose-300 text-xs font-semibold flex items-center gap-1.5 transition-colors border border-rose-800 shrink-0"
        >
        ...
        <button
          ...
          title={!isSanctionAllowed ? 'Mandatory exception must be acknowledged prior to sanction' : 'Recommend facility for formal sanction'}
        >
